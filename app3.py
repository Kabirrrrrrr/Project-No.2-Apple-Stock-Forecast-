# -*- coding: utf-8 -*-
"""
Created on Mon May  8 15:05:00 2023
...
X = apple_stk['Adj Close'].values
size = int(len(X) * 0.75)
train, test = X[0:size], X[size:len(X)]

hwe_model_mul_sea = ExponentialSmoothing(Train["Adj Close"],seasonal="mul",trend="add",seasonal_periods=252).fit() 
...
plt.title('apple stock close value prediction by SARIMA Method', fontsize=16)
...
st.sidebar.header('How many days of Prediction you want:')
...
st.write("Forecasted Results of Apple Stock")
...
def load_result():
    ...
    df1 = pd.DataFrame(result, columns=['Close_Value']) 
...
plt.plot(df['Close_Value'], color='green',label='Forecasted Values')
